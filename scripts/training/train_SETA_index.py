...
# 初始化模型并移动到设备
model = UNetWithAttention(num_classes=3).cuda(0)
# 选用SGD优化器来训练
...
def evaluate_model(model, data_loader, device, num_classes):
    ...
    with torch.no_grad():  # 关闭梯度计算，减少内存占用
        for features, labels, *_ in data_loader:
            ...
            # 计算 F1, ACC 和 MPA
            f1, acc, mpa = compute_metrics(predicted, labels, num_classes)
            f1_list.append(f1)
            acc_list.append(acc)
            mpa_list.append(mpa)
    ...
    return avg_iou, avg_loss, avg_f1, avg_acc, avg_mpa
...
def mytrain(net, train_iter, test_iter, loss, optimizer, num_epochs, scheduler, num_classes):
    ...
    for epoch in range(num_epochs):
        ...
        # 计算训练集上的 mIoU
        train_iou, _, _, _, _ = evaluate_model(net, train_iter, device, num_classes)
        ...
        # 计算验证集上的 mIoU, 损失, F1, ACC 和 MPA
        test_iou, test_loss_avg, avg_f1, avg_acc, avg_mpa = evaluate_model(net, test_iter, device, num_classes)

        epoch_time = time.time() - epoch_start_time  # 记录当前epoch消耗的时间

        print(
            f"Epoch {epoch + 1} --- Train Loss: {train_loss_avg:.3f} --- Train mIoU: {train_iou:.3f} --- Test mIoU: {test_iou:.3f} --- Test Loss: {test_loss_avg:.3f} --- F1 Score: {avg_f1:.3f} --- ACC: {avg_acc:.3f} --- MPA: {avg_mpa:.3f} --- Time: {epoch_time:.2f} sec")
        ...
        if np.mod(epoch + 1, 5) == 0:  # 每5个epoch保存一次模型
            torch.save(net.state_dict(), f'checkpoints/Unet_SETA_{epoch + 1}.pth')
