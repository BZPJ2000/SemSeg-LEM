...
from model_V2.UNet_KAN.archs import UKAN
...
def mytrain(net, train_iter, test_iter, loss, optimizer, num_epochs, scheduler, num_classes):
    ...
    for epoch in range(num_epochs):
        ...
        df.to_excel(os.path.join("savefile", "Unet_kan_convd_camvid1.xlsx"), index=False)

        # 保存模型
        if np.mod(epoch + 1, 5) == 0:  # 每5个epoch保存一次模型
            torch.save(net.state_dict(), f'checkpoints/Unet_kan_convd_{epoch + 1}.pth')
