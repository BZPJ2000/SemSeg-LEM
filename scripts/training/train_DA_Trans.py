...
from model_V2.UNet_DA_Trans.DATransUNet import DA_Transformer as ViT_seg
from model_V2.UNet_DA_Trans.DATransUNet import CONFIGS as CONFIGS_ViT_seg
import time
...
# 加载数据集
dataset = MyDataset(x_dir, y_dir, num_classes=num_classes, resize_shape=(224, 224),augment=False,expansion_factor=5)  # 使用自定义的数据集类加载图像和标签
...
def mytrain(net, train_iter, test_iter, loss, optimizer, num_epochs, scheduler, num_classes):
    ...
    for epoch in range(num_epochs):
        ...
        df.to_excel(os.path.join("savefile", "Unet_DA_camvid1.xlsx"), index=False)

        # 保存模型
        if np.mod(epoch + 1, 5) == 0:  # 每5个epoch保存一次模型
            torch.save(net.state_dict(), f'checkpoints/Unet_DA_{epoch + 1}.pth')
