...
import numpy as np
# from model.UNet_plus.Unet_plus_2 import UnetPlusPlus
from model.UNet_SETA.Unet_ASE_V3 import UNetWithAttention
# 首先，我们要确定使用的设备是GPU还是CPU
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
print(device)
# 设置路径
model_path = r'checkpoints/Unet_SETA_108100.pth'  # 这是你训练好的模型文件路径
image_path = r'E:\A_workbench\A-lab\Unet\Unet_complit\data\ConvertedImages\00022.png'  # 需要进行预测的图像文件路径
output_path = r'E:\A_workbench\A-lab\Unet\Unet_complit\data\predicted2.png'  # 保存预测结果的文件路径

# 加载模型
num_classes = 3  # 因为这是个三分类问题，所以我们设置类数为3
model = UNetWithAttention(num_classes=num_classes).to(device)  # 初始化模型并移动到指定设备（GPU或CPU）
