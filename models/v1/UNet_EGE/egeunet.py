...
if __name__ == "__main__":
    ...
    inputs = torch.randn((1, 3, 224, 224)).to(device)  # 图像输入
    model = EGEUNet(input_channels=3, num_classes=5).to(device)  # 实例化模型
    outputs = model(inputs)
    if isinstance(outputs, tuple):  # 处理 gt_ds=True 返回多个结果的情况
        outputs = outputs[1]  # 获取最终输出
    print(outputs.shape)
