...
class ChannelTransformer(nn.Module):
    def __init__(self,  config, img_size=224, vis=False, channel_num=[96, 192, 384, 768], patchSize=[56, 28, 14, 7]): #
        super().__init__()
...
def get_CTranS_config():
    ...
    config.KV_size = 720 # d1234 is 1440 , d0123 is 720 # KV_size = Q1 + Q2 + Q3 + Q4 # TODO ange
...
if __name__ == "__main__":
    ...
    # 模拟 4 个不同尺度的输入特征图，分别对应 backbone 网络的不同层输出
    # 输入的尺寸和通道数需要与 channel_num 对应
    en1 = torch.randn((1, 96, 56, 56)).to(device)    # 对应 channel_num[0]
    en2 = torch.randn((1, 192, 28, 28)).to(device)   # 对应 channel_num[1]
    en3 = torch.randn((1, 384, 14, 14)).to(device)   # 对应 channel_num[2]
    en4 = torch.randn((1, 768, 7, 7)).to(device)     # 对应 channel_num[3]

    # 初始化模型
    model = ChannelTransformer(config, img_size=224, vis=False, channel_num=[96, 192, 384, 768], patchSize=[56, 28, 14, 7]).to(device)

    # 执行前向传播，获取输出
    x1, x2, x3, x4, attn_weights = model(en1, en2, en3, en4)

    # 打印每个输出特征图的尺寸，确保模型能够正确运行
    print(f"Output x1 shape: {x1.shape}")
    print(f"Output x2 shape: {x2.shape}")
    print(f"Output x3 shape: {x3.shape}")
    print(f"Output x4 shape: {x4.shape}")
