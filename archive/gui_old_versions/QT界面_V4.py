...
def calculate_and_draw_length(image_path):
    ...
    for label in range(1, num_labels):
        ...
        if top_most and bottom_most:
            ...
            if path:
                length_px = len(path)
                length_mm = length_px / 17
                if length_mm < 0.5:
                    continue  # 假设每像素对应 1/17 mm
                draw.line([tuple(top_most[::-1]), tuple(bottom_most[::-1])], fill=(0, 255, 0), width=2)  # 绿色线段
                draw.text((top_most[1], top_most[0]), f"Length: {length_mm:.2f} mm", fill=(0, 255, 0), font=font)  # 绿色文字
