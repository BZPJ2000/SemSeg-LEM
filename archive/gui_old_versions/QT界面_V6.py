...
from Salvage_all_V3 import split_image_by_color_in_directory
from Compositing_original_image_all_V2 import merge_images_by_group
from Markers_all_V3 import process_images
import cv2
...
class ImageSegmentationApp(QtWidgets.QWidget):
    ...
    def predict_image(self):
        if hasattr(self, 'image_path'):
            ...
            split_image_by_color_in_directory(output_dir, self.split_dir)
    ...
    def split_image(self):
        if hasattr(self, 'split_dir'):
            ...
            process_images(self.split_dir, processed_dir, resolution_factor=17)

            # 合并标注结果
            final_dir = os.path.join(processed_dir, 'final_results')
            if not os.path.exists(final_dir):
                os.makedirs(final_dir)
            merge_images_by_group(processed_dir, final_dir, blacken_height=23)
            ...
            for img_name in final_images:
                ...
                # 调用 calculate_and_draw_length 标注路径长度
                calculate_and_draw_length(img_path)
