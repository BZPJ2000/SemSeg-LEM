...
# 初始化模型并移动到设备
model = ACC_UNet(n_channels=n_channels,n_classes=num_classes-1).to(device)
...
def mytrain(net, train_iter, test_iter, loss, optimizer, num_epochs, scheduler, num_classes):
    ...
    for epoch in range(num_epochs):
        ...
        df.to_excel(os.path.join("savefile", "Unet_ACC_camvid1.xlsx"), index=False)

        # 保存模型
        if np.mod(epoch + 1, 5) == 0:  # 每5个epoch保存一次模型
            torch.save(net.state_dict(), f'checkpoints/Unet_ACC_{epoch + 1}.pth')
